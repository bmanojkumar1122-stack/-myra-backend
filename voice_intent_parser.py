...
class VoiceIntentParser:
    ...
    def parse_trusted_intent(text, trusted_manager):
        ...
        - "MYRA, JAANf chrome aur notepad allow karo" / "allow chrome and notepad" → Set allowed apps
        ...
        # Pattern: "allow [app names]" or "[apps] ko allow kar" or "JAANf [apps] allow karo"
        app_matches = re.search(r"(?:allow|JAANf|only)\s+(.+?)(?:\s+(?:allow|karo|kar)|$)", text_lower)
        if app_matches:
