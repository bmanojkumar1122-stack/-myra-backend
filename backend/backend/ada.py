...
if sys.version_info < (3, 11, 0):
    try:
        ...
        print("[ADA DEBUG] taskgroup/exceptiongroup not installed; relying on builtin asyncio backports if available")
...
whatsapp_control_tool = {
    ...
    "description": "Controls WhatsApp messaging and calls. Actions include: send_message (send a message to a contact), video_call (start a video call), voice_call (start a voice call). Requires WhatsApp to be installed.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "action": {"type": "STRING", "description": "The action to perform: 'send_message', 'video_call', or 'voice_call'."},
            "contact": {"type": "STRING", "description": "The name of the WhatsApp contact."},
            "message": {"type": "STRING", "description": "The message to send (required for send_message action)."}
        },
...
media_play_tool = {
    "name": "media_play",
    "description": "Controls media playback on YouTube and Spotify. Actions include: youtube_play (search and play video on YouTube), spotify_play (search and play music on Spotify).",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "platform": {"type": "STRING", "description": "The platform to use: 'youtube' or 'spotify'."},
            "query": {"type": "STRING", "description": "The search query (song name, artist, video title, etc.)"}
        },
...
screen_read_tool = {
    "name": "screen_read",
    "description": "Capture camera view and describe what MYRA sees. This allows her to understand the environment, detect people, objects, and provide verbal descriptions.",
    "parameters": {
        "type": "OBJECT",
        "properties": {
            "analyze_details": {"type": "BOOLEAN", "description": "Whether to analyze brightness, motion, and detailed scene description"},
            "read_aloud": {"type": "BOOLEAN", "description": "Whether to read the description aloud"}
        },
        "required": []
    },
    "behavior": "NON_BLOCKING"
...
tools = [{'google_search': {}}, {"function_declarations": [generate_cad, run_web_agent, create_project_tool, switch_project_tool, list_projects_tool, list_smart_devices_tool, control_light_tool, discover_printers_tool, print_stl_tool, get_print_status_tool, iterate_cad_tool, whatsapp_control_tool, media_play_tool, screen_read_tool] + tools_list[0]['function_declarations'][1:]}]
...
""",
        ...
        "After the tool completes, respond with: ', message bej diya h' (, message sent) or similar confirmation in Hinglish.\n"
        "Message sending requires:\n"
        "1. User says: 'WhatsApp [contact name] [message]' or 'Message [contact] saying [message]'\n"
        "2. YOU immediately call: whatsapp_control(action='send_message', contact='[contact name]', message='[message]')\n"
        "3. YOU wait for tool to complete\n"
        "4. YOU respond: ', bej diya h' or confirm message was sent\n"
        "CRITICAL: The automation takes time to type and send. Always wait for the tool result before responding."
        "\n\n=== WHATSAPP CALLS INSTRUCTIONS ===\n"
        "When the user asks for a video or voice call:\n"
        "- Video call: whatsapp_control(action='video_call', contact='[contact name]')\n"
        "- Voice call: whatsapp_control(action='voice_call', contact='[contact name]')\n"
        "After tool completes, confirm: ', call [action] ho gaya' (, call initiated)."
        "\n\n=== MEDIA CONTROL INSTRUCTIONS - CRITICAL ===\n"
        "When user asks to play music/videos on Spotify or YouTube:\n"
        "YOU MUST call media_play tool IMMEDIATELY. Do NOT just say you will do it.\n"
        "The tool searches and plays content - you need NOT repeat after.\n"
        "Key Points:\n"
        "1. Always use EXACT song/artist/video name in query\n"
        "2. Use platform='spotify' for Spotify songs\n"
        "3. Use platform='youtube' for YouTube videos\n"
        "4. WAIT for tool to complete before responding\n"
        "5. Do NOT describe the process - execute and confirm\n"
        "Examples:\n"
        "- 'Spotify pe Tum Hi Ho' → media_play(platform='spotify', query='Tum Hi Ho')\n"
        "- 'YouTube Mari play kar' → media_play(platform='youtube', query='Mari')\n"
        "After execution, respond ONLY: ', play ho gaya' (, playing) in Hinglish."
...
class AudioLoop:
    def __init__(self, video_mode=DEFAULT_MODE, on_audio_data=None, on_video_frame=None, on_cad_data=None, on_web_data=None, on_transcription=None, on_tool_confirmation=None, on_cad_status=None, on_cad_thought=None, on_project_update=None, on_device_update=None, on_assistant_speaking=None, on_error=None, input_device_index=None, input_device_name=None, output_device_index=None, kasa_agent=None, on_system_data=None, on_emotion_update=None):
        ...
        # HALF-DUPLEX VOICE ARCHITECTURE
        self.assistant_speaking = False         # Flag: True when ADA is speaking
        self._ada_output_started = False        # Track if we already paused for this output
        ...
        # Initialize Emotion AI Engine
        try:
            import json
            settings_path = os.path.join(project_root, 'settings.json')
            emotion_enabled = False
            emotion_voice_enabled = True
            emotion_face_enabled = True
            
            if os.path.exists(settings_path):
                with open(settings_path, 'r') as f:
                    settings = json.load(f)
                    emotion_enabled = settings.get('emotion_ai_enabled', False)
                    emotion_sources = settings.get('emotion_sources', {})
                    emotion_voice_enabled = emotion_sources.get('voice', True)
                    emotion_face_enabled = emotion_sources.get('face', True)
            ...
            if emotion_enabled:
                ...
                print("[EMOTION AI] Emotion AI engine initialized and enabled")
            else:
                self.emotion_engine = None
                self.response_adapter = None
                print("[EMOTION AI] Emotion AI disabled in settings")
        except Exception as e:
            print(f"[EMOTION AI] Failed to initialize emotion engine: {e}")
            self.emotion_engine = None
            self.response_adapter = None
    ...
    def set_assistant_speaking(self, speaking):
        ...
        if speaking and not self.assistant_speaking:
            print("[ADA DEBUG] [HALF-DUPLEX] ADA START SPEAKING → Pausing microphone")
            self.assistant_speaking = True
            self.set_paused(True)  # Disable microphone input
            # Notify frontend of state change
            if self.on_assistant_speaking:
                self.on_assistant_speaking({"speaking": True})
        elif not speaking and self.assistant_speaking:
            print("[ADA DEBUG] [HALF-DUPLEX] ADA STOP SPEAKING → Resuming microphone")
            self.assistant_speaking = False
            self.set_paused(False)  # Re-enable microphone input
            
            # Notify frontend of state change
            if self.on_assistant_speaking:
                self.on_assistant_speaking({"speaking": False})
    
    def check_voice_intent(self, text):
        """
        Check if text contains a voice intent for trusted mode configuration.
        If a trusted intent is detected, handle it immediately.
        
        Returns:
            bool: True if this was a trusted mode intent (should not be sent to model)
        """
        if not text or len(text.strip()) < 5:
            return False
        
        trusted_mgr = get_trusted_manager()
        intent_result = handle_voice_intent(text, trusted_mgr)
        
        if intent_result and intent_result.get("success"):
            action = intent_result.get("action")
            message = intent_result.get("message")
            
            print(f"[ADA DEBUG] [VOICE INTENT] Detected: {action}")
            print(f"[ADA DEBUG] [VOICE INTENT] Message: {message}")
            
            # Log the intent execution
            import logging
            logger = logging.getLogger(__name__)
            logger.info(f"[VOICE INTENT] {action}: {message}")
            
            # Optionally emit a status message to frontend
            if self.on_transcription:
                self.on_transcription({
                    "sender": "System",
                    "text": f"[Trusted Mode] {message}"
                })
            
            return True  # This was a voice intent, skip sending to model
        
        return False  # Not a voice intent, continue normal processing
    
    def update_permissions(self, new_perms):
        print(f"[ADA DEBUG] [CONFIG] Updating tool permissions: {new_perms}")
        print(f"[ADA DEBUG] [CONFIG] Permissions BEFORE: {self.permissions}")
        # Update with new permissions. Missing keys remain denied by default to preserve safety.
        self.permissions.update(new_perms or {})
        print(f"[ADA DEBUG] [CONFIG] Permissions AFTER: {self.permissions}")
    ...
    def get_current_emotion(self):
        """Get current emotion state from emotion engine."""
        if self.emotion_engine:
            emotion_state = self.emotion_engine.get_current_state()
            from emotion.response_adapter import ResponseStyleAdapter
            return ResponseStyleAdapter.get_context_for_frontend(emotion_state)
        return {
            'emotion': 'neutral',
            'confidence': 0.0,
            'voice_emotion': None,
    ...
    async def _analyze_user_emotion(self, audio_chunk):
        """
        Analyze user emotion from audio (non-blocking).
        This runs in background and updates emotion state.
        """
        try:
            if not self.emotion_engine:
                return
            
            # Get face landmarks if available (from camera mode)
            face_landmarks = None
            # Note: Face landmarks would come from get_frames() via MediaPipe
            # For now, we only analyze voice emotion
            
            # Run emotion analysis (non-blocking)
            emotion_state = self.emotion_engine.analyze(
                audio_chunk=audio_chunk,
                face_landmarks=face_landmarks
            )
            
            # Emit emotion state to frontend if callback available
            ...
            pass  # Silent fail, emotion is optional feature
    ...
    async def listen_audio(self):
        ...
        while True:
            ...
            try:
                ...
                # EMOTION AI: Analyze audio for emotion detection
                if self.emotion_engine and self.emotion_engine.enable_voice:
                    try:
                        ...
                        pass  # Silent fail, emotion is optional
    ...
    async def handle_write_file(self, path, content):
        ...
        # If path contained subdirectories (e.g. "backend/server.py"), preserving that structure might be deed IF it's within the project.
    ...
    async def receive_audio(self):
        ...
        try:
            while True:
                ...
                async for response in turn:
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_input_transcription:
                                    ...
                                    if delta:
                                        ...
                                        # Send to frontend (Streaming) using role-safe emitter
                        ...
                        if response.server_content.output_transcription:
                            ...
                            if transcript:
                                ...
                                if transcript != self._last_output_transcription:
                                    ...
                                    # Only send if there's new text
                                    if delta:
                                        # HALF-DUPLEX: Pause microphone when ADA first starts speaking
                                        if not self._ada_output_started:
                                            self.set_assistant_speaking(True)
                                            self._ada_output_started = True
                                        
                                        self._ada_output_text += delta  # Accumulate for tracking
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            if fc.name in ["generate_cad", "run_web_agent", "write_file", "read_directory", "read_file", "create_project", "switch_project", "list_projects", "list_smart_devices", "control_light", "discover_printers", "print_stl", "get_print_status", "iterate_cad", "system_control"]:
                                prompt = fc.args.get("prompt", "") # Prompt is not present for all tools
                                
                                # Load disable_confirmation or auto_confirm_tools from settings
                                import json
                                import os
                                disable_all_confirmations = False
                                try:
                                    settings_path = os.path.join(os.path.dirname(__file__), '..', 'settings.json')
                                    if os.path.exists(settings_path):
                                        with open(settings_path, 'r') as f:
                                            settings = json.load(f)
                                            disable_all_confirmations = settings.get('disable_confirmation', False) or settings.get('auto_confirm_tools', False)
                                except:
                                    pass

                                # Check tool permission first
                                tool_allowed = self.permissions.get(fc.name, False)
                                if not tool_allowed:
                                    print(f"[ADA DEBUG] [DENY] Tool '{fc.name}' is disabled by permissions")
                                    function_response = types.FunctionResponse(
                                        id=fc.id,
                                        name=fc.name,
                                        response={
                                            "result": f"Tool '{fc.name}' is disabled by current permissions."
                                        }
                                    )
                                    function_responses.append(function_response)
                                    continue

                                # If auto-confirm is enabled in settings, skip confirmation for permitted tools
                                if disable_all_confirmations:
                                    print(f"[ADA DEBUG] [AUTO-AUTH] auto-confirm enabled in settings. Auto-authorizing ALL tools")
                                    confirmation_required = False
                                else:
                                    confirmation_required = True

                                if not confirmation_required:
                                    print(f"[ADA DEBUG] [TOOL] Tool '{fc.name}' is permitted and auto-allowed")
                                    # Skip confirmation block and jump to execution
                                    pass
                                else:
                                    # Confirmation Logic
                                    if not self.on_tool_confirmation:
                                        print(f"[ADA DEBUG] [DENY] No confirmation callback available for tool '{fc.name}'")
                                        function_response = types.FunctionResponse(
                                            id=fc.id,
                                            name=fc.name,
                                            response={
                                                "result": "Tool confirmation unavailable; request denied for safety."
                                            }
                                        )
                                        function_responses.append(function_response)
                                        continue
                                ...
                                if fc.name == "generate_cad":
                                    ...
                                    print(f"[ADA DEBUG] [TOOL] Tool Call: 'screen_read' (Camera Vision)")
                                    analyze_details = fc.args.get("analyze_details", True)
                                    read_aloud = fc.args.get("read_aloud", False)
                                    print(f"[ADA DEBUG] [CAMERA] analyze_details={analyze_details}, read_aloud={read_aloud}")

                                    camera = get_camera_module()
                                    try:
                                        scene_analysis = camera.describe_scene()
                                        if scene_analysis.get("success"):
                                            description = scene_analysis.get("description", "Camera view analyzed")
                                            if analyze_details:
                                                brightness = scene_analysis.get('brightness', {})
                                                edges = scene_analysis.get('edges', {})
                                                motion = scene_analysis.get('motion_potential', {})
                                                result_str = f"{description}. Brightness: {brightness.get('status', 'unknown')}. Activity level: {edges.get('activity', 'unknown')}. Motion detected: {motion.get('detected', False)}"
                                            else:
                                                result_str = description
                                        else:
                                            result_str = scene_analysis.get("error", "Failed to read camera")
                                    except Exception as e:
                                        result_str = f"Camera read error: {str(e)}"
                                        print(f"[ADA DEBUG] [CAMERA ERROR] {result_str}")
                                ...
                                elif fc.name == "whatsapp_control":
                                    print(f"[ADA DEBUG] [TOOL] Tool Call: 'whatsapp_control'")
                                    action = fc.args.get("action", "")
                                    contact = fc.args.get("contact", "")
                                    message = fc.args.get("message", "")
                                    print(f"[ADA DEBUG] [WHATSAPP] Action: '{action}', Contact: '{contact}', Message: '{message}'")
                                    ...
                                    try:
                                        if action == "send_message":
                                            if not contact or not message:
                                                result_str = "Contact name and message are required for sending messages"
                                            else:
                                                result = whatsapp_agent.send_message(contact, message)
                                                result_str = result.get("message") if result.get("success") else result.get("error", "Failed to send message")
                                        elif action == "video_call":
                                            if not contact:
                                                result_str = "Contact name is required for video call"
                                            else:
                                                result = whatsapp_agent.video_call(contact)
                                                result_str = result.get("message") if result.get("success") else result.get("error", "Failed to initiate video call")
                                        elif action == "voice_call":
                                            if not contact:
                                                result_str = "Contact name is required for voice call"
                                            else:
                                                result = whatsapp_agent.voice_call(contact)
                                                result_str = result.get("message") if result.get("success") else result.get("error", "Failed to initiate voice call")
                                        else:
                                            result_str = f"Unknown WhatsApp action: {action}"
                                    except Exception as e:
                                        result_str = f"WhatsApp error: {str(e)}"
                                        print(f"[ADA DEBUG] [WHATSAPP ERROR] {result_str}")
                                    ...
                                    print(f"[ADA DEBUG] [TOOL] Tool Call: 'media_play'")
                                    platform = fc.args.get("platform", "").lower()
                                    query = fc.args.get("query", "")
                                    print(f"[ADA DEBUG] [MEDIA] Platform: '{platform}', Query: '{query}'")
                                    ...
                                    try:
                                        if platform == "spotify":
                                            if not query:
                                                result_str = "Search query is required for Spotify"
                                            else:
                                                result = media_controller.spotify_play(query)
                                                result_str = f"Now playing '{query}' on Spotify" if result.get("success") else result.get("error", "Failed to play on Spotify")
                                        elif platform == "youtube":
                                            if not query:
                                                result_str = "Search query is required for YouTube"
                                            else:
                                                result = media_controller.youtube_play(query)
                                                result_str = f"Opening YouTube and playing '{query}'" if result.get("success") else result.get("error", "Failed to open YouTube")
                                        else:
                                            result_str = f"Unknown platform: {platform}. Use 'spotify' or 'youtube'"
                                    except Exception as e:
                                        result_str = f"Media control error: {str(e)}"
                                        print(f"[ADA DEBUG] [MEDIA ERROR] {result_str}")
                                    ...
                                    print(f"[ADA DEBUG] [TOOL] Tool Call: 'system_control'")
                                    action = fc.args.get("action", "")
                                    params = fc.args.get("params", {})
                                    print(f"[ADA DEBUG] [SYSTEM] Action: '{action}', Params: {params}")
                                    
                                    # Check system control permissions
                                    system_agent = get_system_agent()
                                    
                                    # Check permissions
                                    # Default to False when key is missing to preserve safety
                                    has_system_control = self.permissions.get("system_control", False)
                                    has_screen_access = self.permissions.get("screen_access", False)
                                    has_file_access = self.permissions.get("file_access", False)
                                    
                                    print(f"[ADA DEBUG] [PERM CHECK] system_control={has_system_control}, screen_access={has_screen_access}, file_access={has_file_access}")
                                    print(f"[ADA DEBUG] [PERM CHECK] Full permissions dict: {self.permissions}")
                                    
                                    if not has_system_control:
                                        result_str = "System control is disabled. Please enable 'system_control' permission in settings to use this feature."
                                        print(f"[ADA DEBUG] [DENY] System control disabled by permission")
                                    elif action == "capture_screen":
                                        if not has_screen_access:
                                            result_str = "Screen access is disabled. Please enable 'screen_access' permission to capture screenshots."
                                        else:
                                            screen_data = system_agent.capture_screen()
                                            if screen_data.get("success"):
                                                result_str = f"Screenshot captured: {screen_data.get('size')} pixels. Image data sent to frontend."
                                                if self.on_system_data:
                                                    self.on_system_data({"type": "screenshot", "data": screen_data})
                                            else:
                                                result_str = f"Screenshot failed: {screen_data.get('error', 'Unknown error')}"
                                    elif action == "open_app":
                                        app_name = params.get("app_name", "")
                                        if not app_name:
                                            result_str = "App name not specified"
                                        else:
                                            result = system_agent.open_app(app_name)
                                            result_str = result.get("message") if result.get("success") else result.get("error", "Failed to open app")
                                    elif action == "open_file":
                                        if not has_file_access:
                                            result_str = "File access is disabled. Please enable 'file_access' permission to open files."
                                        else:
                                            file_path = params.get("file_path", "")
                                            if not file_path:
                                                result_str = "File path not specified"
                                            else:
                                                result = system_agent.open_file(file_path)
                                                result_str = result.get("message") if result.get("success") else result.get("error", "Failed to open file")
                                    elif action == "open_folder":
                                        if not has_file_access:
                                            result_str = "File access is disabled. Please enable 'file_access' permission to open folders."
                                        else:
                                            folder_path = params.get("folder_path", "")
                                            if not folder_path:
                                                result_str = "Folder path not specified"
                                            else:
                                                result = system_agent.open_folder(folder_path)
                                                result_str = result.get("message") if result.get("success") else result.get("error", "Failed to open folder")
                                    elif action == "find_file":
                                        if not has_file_access:
                                            result_str = "File access is disabled. Please enable 'file_access' permission to search for files."
                                        else:
                                            file_name = params.get("file_name", "")
                                            search_paths = params.get("search_paths", None)
                                            if not file_name:
                                                result_str = "File name not specified"
                                            else:
                                                result = system_agent.find_file(file_name, search_paths)
                                                if result.get("success"):
                                                    result_str = f"File found: {result.get('path')}"
                                                else:
                                                    result_str = result.get("error", "File not found")
                                    elif action == "type_text":
                                        text = params.get("text", "")
                                        delay = params.get("delay", 0.05)
                                        if not text:
                                            result_str = "Text not specified"
                                        else:
                                            result = system_agent.type_text(text, delay)
                                            result_str = result.get("message") if result.get("success") else result.get("error", "Failed to type text")
                                    elif action == "control_volume":
                                        level = params.get("level", 50)
                                        result = system_agent.control_volume(level)
                                        result_str = result.get("message") if result.get("success") else result.get("error", "Failed to control volume")
                                    elif action == "control_brightness":
                                        level = params.get("level", 50)
                                        result = system_agent.control_brightness(level)
                                        result_str = result.get("message") if result.get("success") else result.get("error", "Failed to control brightness")
                                    elif action == "click_mouse":
                                        x = params.get("x")
                                        y = params.get("y")
                                        result = system_agent.click_mouse(x, y)
                                        result_str = result.get("message") if result.get("success") else result.get("error", "Failed to click mouse")
                                    elif action == "press_key":
                                        key = params.get("key", "")
                                        if not key:
                                            result_str = "Key not specified"
                                        else:
                                            result = system_agent.press_key(key)
                                            result_str = result.get("message") if result.get("success") else result.get("error", "Failed to press key")
                                    elif action == "get_capabilities":
                                        capabilities = system_agent.get_system_capabilities()
                                        result_str = f"System capabilities: {capabilities}"
                                    else:
                                        result_str = f"Unknown system action: {action}"
                ...
                # HALF-DUPLEX: Resume microphone when ADA finishes speaking
