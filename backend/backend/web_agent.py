...
class WebAgent:
    ...
    # --- Synchronous desktop-oriented API expected by server code ---
    def run(self, command: str) -> dict:
        ...
        try:
            ...
            if 'google' in lower or 'search' in lower:
                # If explicit youtube word present, handle above. Otherwise google.
            ...
            if 'open' in lower or 'website' in lower or 'http' in lower or 'www.' in lower:
                ...
                for p in parts:
                    if p.startswith('http') or p.startswith('www.'):
                        ...
                        break
                if not url:
                    # take word after 'open'
                    if 'open' in lower:
                        try:
                            url = parts[parts.index('open') + 1]
                        except Exception:
                            url = None
                ...
                return {'success': False, 'error': 'No URL found to open'}
            ...
            if 'type' in lower or 'write' in lower:
                ...
                for kw in ['type', 'write']:
                    if kw in lower:
                        try:
                            content = cmd.split(kw, 1)[1].strip()
                            self.type_text(content)
                            return {'success': True, 'action': 'type_text', 'text': content}
                        except Exception as e:
                            return {'success': False, 'error': str(e)}
    ...
    def google_search(self, query: str) -> dict:
        try:
            # extract query words after 'search' or use full string
            q = query
            for kw in ['search', 'google', 'find', 'look up', 'find out']:
                if kw in q.lower():
            ...
            return {'success': True, 'action': 'google_search', 'query': q, 'url': url, 'page': page}
        except Exception as e:
            return {'success': False, 'error': str(e)}
    ...
    def youtube_search(self, query: str) -> dict:
        try:
            q = query
            for kw in ['search', 'youtube', 'play', 'on youtube']:
                if kw in q.lower():
                    q = q.lower().split(kw, 1)[1].strip()
                    break
            if not q:
            ...
            time.sleep(0.5)
            page = self.read_current_page()
            return {'success': True, 'action': 'youtube_search', 'query': q, 'url': url, 'page': page}
        except Exception as e:
            return {'success': False, 'error': str(e)}

    def read_current_page(self) -> dict:
        """Attempt to read current browser window title and infer URL when possible."""
        try:
            win = gw.getActiveWindow()
            if not win:
                # fallback: try to find a browser window
            ...
            # Try to extract URL from window title (best-effort)
            url = None
            # Common pattern: "Page Title - site.com - Google Chrome"
            parts = title.split(' - ')
            if parts:
                # last part often browser name; search for something that looks like domain
                for p in parts:
    ...
    def click(self, selector: str = None) -> dict:
        """Click at coordinates or the current mouse position. selector may be 'left'/'right' or coordinates 'x,y'."""
        try:
            if selector and isinstance(selector, str) and ',' in selector:
                parts = selector.split(',')
                x = int(parts[0].strip())
                y = int(parts[1].strip())
                pyautogui.click(x, y)
            elif selector in ['right', 'right-click']:
    ...
    def scroll(self, direction: str = 'down', amount: int = 500) -> dict:
        try:
            if direction == 'down':
                ...
                # horizontal scroll not directly supported on all systems; approximate
