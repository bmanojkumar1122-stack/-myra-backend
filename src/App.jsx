...
function App() {
    ...
    // Auth State
    const [isAuthenticated, setIsAuthenticated] = useState(true); // Force authenticated (disable lock UI)

    // Initialize from LocalStorage to prevent flash of UI
    const [isLockScreenVisible, setIsLockScreenVisible] = useState(false); // Lock screen disabled
    ...
    useEffect(() => {
        // Force disable face auth on startup to avoid the lock screen blocking Electron during development
        try {
            localStorage.setItem('face_auth_enabled', 'false');
            setFaceAuthEnabled(false);
            setIsLockScreenVisible(false);
        } catch (e) {
            console.warn('Could not update face_auth_enabled in localStorage:', e);
        }
    }, []);
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'MYRA Started') {
                setStatus('Model Connected');
            } else if (data.msg === 'MYRA Stopped') {
                setStatus('Connected');
            }
        });
        ...
        socket.on('emotion_state', (data) => {
            console.log('Emotion state:', data);
            setEmotionState(data);
        });
        ...
        // Handle system data (screenshots, etc.)
        socket.on('system_data', (data) => {
            console.log("Received System Data:", data.type);
            if (data.type === 'screenshot') {
                // Display screenshot
                addMessage('System', '📸 Screenshot captured by MYRA');
                // You can create a new window component or modal to display the screenshot
                if (data.data && data.data.data) {
                    // Store screenshot temporarily
                    localStorage.setItem('lastScreenshot', data.data.data);
                    localStorage.setItem('lastScreenshotTime', new Date().toLocaleTimeString());
                }
            }
        });
        ...
        // HALF-DUPLEX: Handle assistant speaking state
        socket.on('assistant_speaking', (data) => {
            ...
            // Visual feedback (optional: update UI to show mic disabled)
            if (isSpeaking) {
    ...
    // Auto-start camera and microphone on app startup (tries to select default devices)
    useEffect(() => {
        (async () => {
            try {
                ...
                if (audioIn && !selectedMicId) {
                    setSelectedMicId(audioIn.deviceId);
                }
                if (videoIn && !selectedWebcamId) {
                    setSelectedWebcamId(videoIn.deviceId);
                }

                // Start streams if available
                if (videoIn) {
                    await startVideo();
                }
                if (audioIn) {
                    startMicVisualizer(audioIn.deviceId);
                }
            } catch (e) {
                console.error('Auto-start media failed', e);
            }
        })();
    }, []);
            ...
            if (results.landmarks && results.landmarks.length > 0) {
                ...
                if (snapStateRef.current.isSnapped) {
                    ...
                    if (closest && minDist < SNAP_THRESHOLD) {
                        ...
                        // SNAP HIGHLIGHT Logic - PINK THEME
                        ...
                        closest.el.style.boxShadow = '0 0 20px rgba(236, 72, 153, 0.6)';
                        closest.el.style.backgroundColor = 'rgba(236, 72, 153, 0.2)';
                        closest.el.style.borderColor = 'rgba(236, 72, 153, 1)';
                    }
                }
    ...
    const drawSkeleton = (ctx, landmarks) => {
        ctx.strokeStyle = '#FF1493'; // PINK/RED HACKING COLOR
        ctx.lineWidth = 2;
    ...
    return (
        <div className="h-screen w-screen bg-black text-pink-100 font-mono overflow-hidden flex flex-col relative selection:bg-pink-900 selection:text-white">
            ...
            {isVideoOn && isHandTrackingEnabled && (
                <div
                    className={`fixed w-6 h-6 border-2 rounded-full pointer-events-none z-[100] transition-transform duration-75 ${isPinching ? 'bg-pink-500 border-pink-500 scale-75 shadow-[0_0_15px_rgba(236,72,153,0.8)]' : 'border-pink-500 shadow-[0_0_10px_rgba(236,72,153,0.5)]'}`}
                    style={{
                        left: cursorPos.x,
                        top: cursorPos.y,
            ...
            {/* Background Grid/Effects - HACKING STYLE */}
            <div
                className="absolute inset-0 bg-gradient-to-br from-red-950 via-black to-pink-950 z-0 pointer-events-none"
                style={{ opacity: 1 }}
            ></div>
            ...
            {/* Ambient Glow - PULSING PINK */}
            <div
                className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-pink-700/20 rounded-full blur-[120px] pointer-events-none animate-pulse"
            />

            {/* Top Bar (Draggable) - PINK THEME */}
            <div className="z-50 flex items-center justify-between p-2 border-b-2 border-pink-500/60 bg-black/60 backdrop-blur-md select-none sticky top-0" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-4 pl-2">
                    <h1 className="text-xl font-bold tracking-[0.2em] text-pink-400 drop-shadow-[0_0_10px_rgba(236,72,153,0.8)] animate-pulse">
                        MYRA
                    </h1>
                    <div className="text-[10px] text-pink-600 border border-pink-800 px-1 rounded">
                        V2.0.0
                    </div>
                    {/* FPS Counter */}
                    {isVideoOn && (
                ...
                <div className="flex items-center gap-2 pr-2" style={{ WebkitAppRegion: 'no-drag' }}>
                    {/* Emotion widget */}
                    <EmotionWidget emotion={emotionState} />

                    {/* Live Clock */}
                    <div className="flex items-center gap-1.5 text-[11px] text-pink-300/70 font-mono px-2">
                        <Clock size={12} className="text-pink-500/50" />
                        <span>{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <button onClick={handleMinimize} className="p-1 hover:bg-pink-900/50 rounded text-pink-500 transition-colors">
                        <Minus size={18} />
                    </button>
                    <button onClick={handleMaximize} className="p-1 hover:bg-pink-900/50 rounded text-pink-500 transition-colors">
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                <div
                    ...
                    className={`absolute flex items-center justify-center transition-all duration-200 
                        backdrop-blur-xl bg-black/50 border-2 border-pink-500/30 shadow-2xl overflow-visible
                        ${isModularMode ? (activeDragElement === 'visualizer' ? 'ring-2 ring-pink-500 bg-pink-500/20' : 'ring-1 ring-pink-500/30 bg-pink-500/10') + ' rounded-2xl pointer-events-auto' : 'rounded-2xl pointer-events-none'}
                    `}
                    style={{
                ...
                >
                    ...
                    {isModularMode && <div className={`absolute top-2 right-2 text-xs font-bold tracking-widest z-20 ${activeDragElement === 'visualizer' ? 'text-pink-500' : 'text-pink-500/50'}`}>VISUALIZER</div>}
                </div>

                {/* Video Feed Overlay */}
                {/* Floating Project Label */}
                <div className="absolute top-[70px] left-1/2 -translate-x-1/2 text-pink-500 text-xs font-mono tracking-widest pointer-events-none z-50 bg-black/50 px-2 py-1 rounded backdrop-blur-sm border border-pink-500/20">
                ...
                >
                    ...
                    <div className="relative border border-pink-500/30 rounded-lg overflow-hidden shadow-[0_0_20px_rgba(236,72,153,0.1)] w-80 aspect-video bg-black/80">
                        ...
                        <div className="absolute top-2 left-2 text-[10px] text-pink-400 bg-black/60 backdrop-blur px-2 py-0.5 rounded border border-pink-500/20 z-10 font-bold tracking-wider">CAM_01</div>
                ...
                {showCadWindow && (
                    <div
                        ...
                        backdrop-blur-xl bg-black/40 border border-pink-500/30 shadow-2xl overflow-hidden rounded-2xl
                        ${activeDragElement === 'cad' ? 'ring-2 ring-pink-500 bg-pink-500/10' : ''}
                    `}
                        style={{
                    ...
                    >
                        ...
                        <div
                            data-drag-handle
                            className="h-8 bg-gray-900/80 border-b border-pink-500/20 flex items-center justify-between px-3 cursor-grab active:cursor-grabbing shrink-0"
                        >
                            <span className="text-xs font-bold tracking-widest text-pink-500/70">CAD PROTOTYPE</span>
                            <button
                ...
                {showBrowserWindow && (
                    <div
                        id="browser"
                        className={`absolute flex flex-col transition-all duration-200 
                        backdrop-blur-xl bg-black/40 border border-pink-500/30 shadow-2xl overflow-hidden rounded-lg
                        ${activeDragElement === 'browser' ? 'ring-2 ring-pink-500 bg-pink-500/10' : ''}
                    `}
                        style={{
